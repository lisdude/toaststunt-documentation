...
* Cleaned up FileIO documentation and function help to match source behavior, including `file_stat()`, `file_mode()`, `file_type()`, `file_openmode()`, `file_readlines()`, `file_count_lines()`, and FHANDLE terminology
* Clarified conditional availability and permission behavior for SQLite, `finished_tasks()`, `connection_info()` TLS data, and related function-help entries
...
## 2026.06.24
* Added manual documentation explaining map key ordering and how `mapkeys()` / `mapvalues()` return entries.
...
* Updated function help for the `generate_json()` third argument.

## [1.2.1] - 2026.01.22 11:32PM
...
## 2025.10.07
* Corrected `mapdelete()` function help syntax for single-key and list-of-keys arguments.

## 2025.09.30
* Updated manual and function help through ToastStunt 2.7.3 behavior.
* Documented `occupants()` inverse-match/threading behavior, `clear_ancestor_cache()` removal, TCP keep-alive connection options, `curl()` DICT support, `listen()` interface support, listener interface reporting, `trusted_proxies`, `proxy_rewrite` removal, and `--no-ipv6`.
* Updated manual copyright years.

## 2025.06.22
* Updated function help for `mapdelete()` list-of-keys support.
* Added `shutdown()` function help, including clean and unclean shutdown behavior.
...
## 2022.06.06
* Added `pcre_replace()` function help for capture group replacement placeholders.

## 2022.06.05
* Corrected `delete_verb()` function help parsing/formatting.

## 2022.04.02
* Updated function help for verb-related builtins that accept either verb names or verb indexes from `verbs()`.
...
## 2022.01.08 10:37PM
* Added or updated function help for cache stats, object creation, hashing, base64, server logging, waifs, SQLite, types, reverse, locations, finished tasks, explode, parent/child helpers, and related builtins.
...
## 2022.01.03 12:25PM
* Marked the `is_member()` case-sensitive argument as optional.
...
## 2022.01.03 10:15AM
* Added the ToastStunt help index.

## 2022.01.02 5:46PM
* Added additional function help files.

## 2022.01.02 3:59PM
* Added FileIO helper entries for `file_count_lines()`, `file_handles()`, and `file_grep()`.
...
## 2022.01.02 2:42AM
* Added function help for `random()`, hash helpers, `descendents`, and additional ToastStunt builtins.

## [0.0.1] - 2022.01.01 10:17pm
* Added beta version of ToastStunt Programmers Manual

## 2021.06.17
* Added FileIO function help.

## 2021.06.10
* Updated function help for `is_member()` case-sensitive behavior.
* Removed `process_id()` help after server removal.

## [0.0] - 2021.04.20 4:29pm
* Added new fork on documentation for ToastStunt

## 2021.01.25
* Updated function help for `mapvalues()`, `switch_player()`, `sqlite_query()`, SQLite overview, and `ftime()`.
* Added help for `sqlite_interrupt()`, `queued_tasks()`, `move()`, `listen()`, and `open_network_connection()`.
